...
at::Tensor forward_fwht_cpu(at::Tensor input)
{
    ...
    TORCH_CHECK(shape.size() == 2, "Input tensor should be 2D. Please, reshape it as 2D.\n")
    int batch_size = shape[0];
    int log2d = (int)log2(shape[1]);
    TORCH_CHECK((float)log2d == log2(shape[1]), "Number of input feature has to be an integer power of 2.\n")
...
at::Tensor forward_fwht_gpu(at::Tensor input)
{
    ...
    int log2d = (int)log2(shape[1]);
    TORCH_CHECK((float)log2d == log2(shape[1]), "Number of input feature has to be an integer power of 2.\n")
    //at::Tensor output = input.clone(); // TODO: may be avoided
    //CHECK_INPUT(output);

    return forward_fwht_cuda(input);
}
#endif
...
{
    m.def("forward", &forward_fwht, "Implements the Fast Walsh Hadamard transform on CPU or GPU (if available)");
}
