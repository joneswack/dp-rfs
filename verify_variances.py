...
if __name__ == "__main__":
    ...
    # real_var_function = lambda X, p, D: var_rademacher_comp_real(X, p, D)
    # # real_var_function = lambda X, p, D: var_tensor_srht_comp_real(X, p, D, full_cov=False)
    # comp_var_function = lambda X, p, D: var_tensor_srht_comp_real(X, p, D, full_cov=False)
    ...
    #     #placeholder[:, train_data.shape[1]-1] = np.sqrt(bias)
    #     train_data = placeholder

    #     for degree in [1, 2] + list(range(3, 11, 2)): # 11
    ...
    #         var_comp, _ = comp_var_function(train_data, p=degree, D=0.5*power_2_pad)
    #         var_real = real_var_function(train_data, p=degree, D=0.5*power_2_pad)
